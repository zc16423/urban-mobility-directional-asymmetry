...
ref_folder = os.path.join(ROOT_DIR, "cbg_socioeconomic_attributes")
if not os.path.exists(ref_folder):
    ref_folder = os.path.join(ROOT_DIR, "cbg_socioeconomic_poi")
...
if __name__ == "__main__":
    gz_path = os.path.join(SCRIPT_DIR, "Fig3B_active_links.csv.gz")
    csv_path = os.path.join(SCRIPT_DIR, "Fig3B_active_links.csv")
    stats_path = os.path.join(SCRIPT_DIR, "Fig3B_blockage_stats.csv")

    if os.path.exists(gz_path) and os.path.exists(stats_path):
        print(f"Loading precomputed link data from {os.path.basename(gz_path)}...")
        df_plot = pd.read_csv(gz_path)
        stats_df = pd.read_csv(stats_path, index_col=0)
        blockage_stats = stats_df.to_dict()
    elif os.path.exists(csv_path) and os.path.exists(stats_path):
        print(f"Loading precomputed link data from {os.path.basename(csv_path)}...")
