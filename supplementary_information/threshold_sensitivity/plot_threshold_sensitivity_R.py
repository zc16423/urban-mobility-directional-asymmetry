...
# -*- coding: utf-8 -*-
"""
Plotting Script: Threshold Sensitivity Analysis - Realization Ratio (R)
Reproduces: Supplementary Figure S5 (Realization Ratio R Sensitivity)
Data Source: threshold_sensitivity_R_data.csv
"""
...
def plot_threshold_sensitivity_R():
    # Base directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_file = os.path.join(script_dir, "threshold_sensitivity_R_data.csv")
    out_png = os.path.join(script_dir, "threshold_sensitivity_R.png")
    ...
    for col_idx, model in enumerate(models):
        ...
        for d, color, marker, label in [("HPM_to_LPW", COLOR_H2L, 'o', "HPM \u2192 LPW"), 
            ...
            ax.errorbar(
                dsub["poverty_threshold"], dsub["mean_R"], yerr=dsub["sem_R"],
    ...
    for col_idx, model in enumerate(models):
        ...
        for d, color, marker, label in [("HPM_to_LPW", COLOR_H2L, 'o', "HPM \u2192 LPW"), 
            ...
            ax.errorbar(
                dsub["composition_threshold"], dsub["mean_R"], yerr=dsub["sem_R"],
                color=color, marker=marker, markersize=6, linewidth=2.0,
                capsize=3, capthick=1.2, label=label if col_idx == 0 else ""
            )
