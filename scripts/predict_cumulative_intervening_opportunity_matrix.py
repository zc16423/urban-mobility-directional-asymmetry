...
BASE_POP_PATH = ROOT_DIR / "cbg_socioeconomic_attributes"
if not BASE_POP_PATH.exists():
    BASE_POP_PATH = ROOT_DIR / "cbg_socioeconomic_poi"
BASE_OPPORTUNITY_PATH = ROOT_DIR / "basic_data" / "cumulative_intervening_opportunity_matrices"
BASE_OUTPUT_PATH = ROOT_DIR / "basic_data" / "iom_day"
...
def predict_interzonal_od_fast(origin_outflows, destination_pop, opportunity_matrix, lambda_param):
    """
    Generate production-constrained cumulative-opportunity predictions.

    The diagonal is excluded so the model predicts cross-CBG flows only.
    """
    lambda_s = -lambda_param * opportunity_matrix
    lambda_sm = -lambda_param * (opportunity_matrix + destination_pop)

    raw_score = np.maximum(0, np.exp(lambda_s) - np.exp(lambda_sm))
    np.fill_diagonal(raw_score, 0)

    row_sums = raw_score.sum(axis=1, keepdims=True)
    row_sums[row_sums == 0] = 1e-12
    prob_matrix = raw_score / row_sums

    return origin_outflows * prob_matrix
...
def compute_metrics_fast(y_true, y_pred):
    ...
    numerator = 2 * np.sum(np.minimum(yt, yp))
    denominator = np.sum(yt) + np.sum(yp)
    cpc = numerator / denominator if denominator > 0 else 0

    return mae, rmse, corr, cpc
...
def fit_daily_city(city_path):
    ...
    opp_file = BASE_OPPORTUNITY_PATH / f"{city}_opportunity_matrix.csv"
    ...
    for idx, od_file in enumerate(od_files):
        ...
        inter_real = real_mat.copy()
        np.fill_diagonal(inter_real, 0)
        origin_outflows = np.sum(inter_real, axis=1, keepdims=True)
        ...
        for start_l in [0.0001, 0.001, 0.01]:
            try:
                res = opt.minimize(
                    ...
                    x0=[start_l],
                    args=(origin_outflows, destination_acs, opp_mat, inter_real),
                    method="L-BFGS-B",
                    bounds=[(1e-9, 5.0)],
                    options=opts,
                )
        ...
        mae, rmse, corr, cpc = compute_metrics_fast(inter_real[mask], pred_matrix[mask])

        output_df = pd.DataFrame(pred_matrix, index=b_ids, columns=b_ids).round(6)
        output_df.index.name = "origin_geoid"
        output_df.to_csv(city_matrix_dir / f"PathIO_{od_file.name}", encoding="utf-8-sig")
        ...
        city_results.append(
            {
                ...
                "correlation": corr,
                "cpc": cpc,
                "total_observed_flow": np.sum(real_mat),
                "total_interzonal_flow": np.sum(inter_real),
            }
        )

        if (idx + 1) % 10 == 0:
            print(f"    Progress: {idx + 1}/{len(od_files)} | Lambda: {lambda_opt:.6f} | CPC: {cpc:.4f}")
            gc.collect()
...
def main():
    ...
    print(f"Starting cumulative-opportunity daily fitting for {len(city_paths)} city inputs.")
    ...
    print("\nAll cumulative-opportunity city fitting is complete.")
