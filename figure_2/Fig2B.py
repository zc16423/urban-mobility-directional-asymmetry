# -*- coding: utf-8 -*-
"""
Created on Mon Jan 26 10:26:27 2026
...
DATA_DIR = os.path.join(SCRIPT_DIR, "Fig2BC_city", "HPM_to_LPW")
OUTPUT_DIR = SCRIPT_DIR
os.makedirs(OUTPUT_DIR, exist_ok=True)

BASELINE_COLUMN = "log_LPM"
FLOW_SHARE_COLUMN = "log_flow_share_HPM_to_LPW"
...
# log_LPM is used because it is the verified current baseline column for this panel;
# its semantic naming should be reviewed by the user.
reference_values = (
...
colors = plt.cm.Reds(np.linspace(0.3, 0.9, n_cities))
...
for i, city in enumerate(city_order):
    if city in reference_values:
        ...
        ax.fill_betweenx([y_bottom, y_top], x_start, x_end, color="#d62728", alpha=0.2, zorder=1)
        ...
        if first_ref:
            ax.vlines(
                ...
                linewidth=2.0,
                alpha=1.0,
                label="Share of City Population in LPW CBGs (population baseline)",
            )
            first_ref = False
        else:
            ax.vlines(ref_value, y_pos, y_top, color="#1f77b4", linewidth=2.0, alpha=1.0)
...
target_percentages = [3, 5, 10, 20, 30, 40, 50, 70]
...
minor_percentages = [4, 6, 7, 8, 9, 12, 14, 16, 18, 25, 35, 45, 60]
minor_xticks = np.log(np.array(minor_percentages) / 100)
ax.xaxis.set_minor_locator(FixedLocator(minor_xticks))

ax.set_xlabel("Flow ratio ($P_{HL}$)", fontsize=24, labelpad=15)
...
png_path = os.path.join(OUTPUT_DIR, "Fig2B.png")
svg_path = os.path.join(OUTPUT_DIR, "Fig2B.svg")
