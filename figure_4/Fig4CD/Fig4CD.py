...
if os.path.exists(INPUT_FILE):
    ...
    print(f"Raw input file '{os.path.basename(INPUT_FILE)}' not found; using precomputed summaries in Fig4CD_data/")
    df_active = None
...
def plot_city_equalized_relationship(
    ...
    if data is not None:
        ...
        print(f"  Loaded precomputed summary from: {summary_path}")
        summary = pd.read_csv(summary_path)
    else:
        raise FileNotFoundError(
            f"Neither raw data nor precomputed summary file found: {summary_path}"
        )

    print(f"  Valid bins: {len(summary)}")
    if "n_cities" in summary.columns:
        print(f"  Cities per bin range: {summary['n_cities'].min()} - {summary['n_cities'].max()}")
    if "min_city_bin_n" in summary.columns and "max_city_bin_n" in summary.columns:
        print(
